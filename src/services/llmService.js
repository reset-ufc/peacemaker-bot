...
function getPrompts() {
    ...
    return JSON.parse(fs.readFileSync(filePath, "utf-8"));e
}
...
async function getCommentSuggestions(toxicComment, language) {
    try {
        ...
        const classification = await llmRequest(prompts.prompt_classification_en, toxicComment);

        return {
            friendlyComment: JSON.parse(friendlyComment.choices[0].message.content.replace(/[\u2018\u2019]/g, "'").replace(/[\u201c\u201d]/g, '"')),
            classification: JSON.parse(classification.choices[0].message.content.replace(/[\u2018\u2019]/g, "'").replace(/[\u201c\u201d]/g, '"'))
        };
    } catch (error) {
